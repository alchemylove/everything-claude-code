---
name: cpp-testing
description: C++ テストの作成/更新/修正、GoogleTest/CTest 設定、失敗・フレーキーテスト診断、coverage/sanitizers 追加時のみ使用。C++ testing, GoogleTest, CTest.
---

# C++ Testing（エージェントスキル） (C++ Testing)
...
### 基本的なユニットテスト（gtest） (Basic Unit Test)
...
### CMake/CTest クイックスタート (CMake/CTest Quickstart)
...
## フレーキーテストのガードレール (Flaky Tests Guardrails)
...
## オプションの付録: ファジングとプロパティテスト (Optional Appendix: Fuzzing / Property Testing)
...
## GoogleTest の代替 (Alternatives to GoogleTest)
