...
description: 自動ループ — 自律 Claude Code loop のパターンとアーキテクチャ。シンプルな sequential pipeline から RFC 駆動マルチエージェント DAG システムまで。
origin: ECC
---

# 自動ループスキル (Autonomous Loops Skill)
...
## ループパターンスペクトラム (Loop Pattern Spectrum)
...
## 1. 順序パイプライン（`claude -p`） (1. Sequential Pipeline)
...
```bash
#!/bin/bash
# daily-dev.sh — 機能ブランチの順序パイプライン (Key Design Principles)

set -e

# ステップ1：機能を実装 (Variations)
claude -p "docs/auth-spec.mdのスペックを読む。src/auth/にOAuth2ログインを実装します。TDDを最初にテストを書いてください。新しいドキュメントファイルを作成しないでください。"

# ステップ2：De-sloppify（クリーンアップパス） (2. NanoClaw REPL)
...
# ステップ3：検証 (How It Works)
claude -p "完全なビルド、lint、型チェック、テストスイートを実行します。失敗を修正します。新しい機能を追加しないでください。"

# ステップ4：コミット (When NanoClaw vs Sequential Pipeline)
