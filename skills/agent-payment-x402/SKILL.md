...
# エージェント決済実行（x402） (Agent Payment Execution)
...
### ノンカストディアルウォレット (Non-Custodial Wallets)
...
### オプション A: agentwallet-sdk（Base / マルチチェーン） (Option A: agentwallet-sdk)
...
### 利用可能なツール（エージェント呼び出し可能） (Available Tools)
...
### オプション B: OKX エージェント決済プロトコル（X Layer） (Option B: OKX Agent Payments Protocol)
...
### MCP クライアントでのバジェット強制 (Budget enforcement in an MCP client)
