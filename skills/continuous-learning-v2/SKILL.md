...
description: フックでセッションを観察し、信頼度スコア付きのアトミックなインスティンクトを作成し、skills/commands/agents へ進化させるインスティンクトベース学習。v2.1 はプロジェクト横断汚染を防ぐ project-scoped instincts を追加。continuous learning, instinct, session learning.
version: 2.0.0
---

# Continuous Learning v2 - インスティンクトベースアーキテクチャ (Continuous Learning v2.1 - Instinct)

Claude Codeセッションを信頼度スコアリング付きの小さな学習済み行動である「インスティンクト」を通じて再利用可能な知識に変える高度な学習システム。

## v2の新機能 (When to Activate)
...
## インスティンクトモデル (What's New in v2.1)
...
## クイックスタート (The Instinct Model)

### 1. 観察フックを有効化 (How It Works)
...
### 2. ディレクトリ構造を初期化 (Project Detection)
...
### 3. インスティンクトコマンドを使用 (Data Directory)
...
## Skill Creator との統合 (Skill Creator Integration)
