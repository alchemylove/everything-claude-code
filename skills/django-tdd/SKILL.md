...
# Django テスト駆動開発 (Django Testing with TDD)
...
## DjangoのためのTDDワークフロー (TDD Workflow for Django)

### Red-Green-Refactorサイクル (Red-Green-Refactor Cycle)
...
### pytest 設定 (pytest Configuration) (pytest Configuration)
...
### Djangoビューテスト (Django View Testing)
...
## DRF APIテスト (DRF API Testing)

### シリアライザーテスト (Serializer Testing)
...
### API ViewSetテスト (API ViewSet Testing)
...
## モッキングとパッチング (Mocking and Patching)
...
### メール送信のモック (Mocking Email Sending)
