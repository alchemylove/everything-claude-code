...
description: アーキテクチャ決定記録 — Claude Code セッション中のアーキテクチャ決定を構造化 ADR として記録。決定タイミングの自動検出、context、検討した代替案、根拠を記録。将来の開発者が codebase の形状の理由を理解できる ADR log を維持。
origin: ECC
---
...
### 新しいADRをキャプチャする (Capturing a New ADR)
...
### 既存のADRを読む (Reading Existing ADRs)
...
### ADRディレクトリ構造 (ADR Directory Structure)
...
## 決定検出シグナル (Decision Detection Signals)
...
## 記録する価値のある決定カテゴリ (Categories of Decisions Worth Recording)
...
## 他のスキルとの統合 (Integration with Other Skills)
