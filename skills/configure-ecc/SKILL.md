---
name: configure-ecc
description: ECC（Everything Claude Code）のインタラクティブインストーラ — スキルとルールの選択インストール、パス検証、任意で最適化。configure ecc, install ecc, setup everything claude code.
---

# Everything Claude Code の設定 (Configure Everything Claude Code)
...
## ステップ 0: ECC リポジトリのクローン (Step 0: Clone ECC Repository)
...
## ステップ 1: インストールレベルの選択 (Step 1: Choose Installation Level)
...
## ステップ 2: スキルの選択とインストール (Select & Install Skills)
...
```bash
# コアスキルは .agents/skills/ 配下にあります (Step 2: Select & Install Skills)
cp -R "$ECC_ROOT/.agents/skills/<skill-name>" "$TARGET/skills/"

# ニッチスキルは skills/ 配下にあります (2a: Choose Scope)
...
## ステップ 3: ルールの選択とインストール (Select & Install Rules)
...
## ステップ 4: インストール後の検証 (Post-Install Verification)

インストール後、以下の自動チェックを実行します：

### 4a: ファイルの存在確認 (File Existence Check)
...
### 4b: パス参照のチェック (Path Reference Check)
...
### 4c: スキル間の相互参照のチェック (Cross-Reference Check)
...
## ステップ 5: インストールされたファイルの最適化（オプション） (Optimize Installed Files)
...
## ステップ 6: インストールサマリー (Installation Summary)
...
```
## ECC インストール完了 (ECC Installation Complete)
...
### "スキルが Claude Code に認識されません" ("Skill not recognized by Claude Code")
...
### "プロジェクトレベルのインストール後のパス参照エラー" ("Path reference errors after project-level install")
