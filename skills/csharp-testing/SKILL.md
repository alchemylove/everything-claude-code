---
name: csharp-testing
description: xUnit、FluentAssertions、mocking、integration tests、テスト構成の C# / .NET テストパターン。C# testing, xUnit, .NET.
origin: ECC
---

# C#テストパターン (C# Testing Patterns)
...
## テストフレームワークスタック (Test Framework Stack)
...
### Theoryによるパラメータ化テスト (Parameterized Tests with Theory)
...
## NSubstituteによるモッキング (Mocking with NSubstitute)
...
## ASP.NET Core統合テスト (ASP.NET Core Integration Tests)

### WebApplicationFactoryのセットアップ (WebApplicationFactory Setup)
...
### Testcontainers (Testcontainers)によるテスト (Testing with Testcontainers)
...
## よくあるアンチパターン (Common Anti-Patterns)
