...
# AI リグレッションテスト (AI Regression Testing)
...
## サンドボックスモード API テスト (Sandbox-Mode API Testing)

AI フレンドリーなアーキテクチャを持つほとんどのプロジェクトにはサンドボックス/モックモードがあります。これが高速な DB フリー API テストの鍵です。

### セットアップ（Vitest + Next.js App Router） (Setup)
...
### Next.js API ルート用テストヘルパー (Test Helper for Next.js API Routes)
...
### リグレッションテストの作成 (Writing Regression Tests)
...
### サンドボックス/本番のパリティテスト (Testing Sandbox/Production Parity)
...
## バグチェックワークフローへのテスト統合 (Integrating Tests into Bug-Check Workflow)

### カスタムコマンド定義 (Custom Command Definition)
...
## 一般的な AI リグレッションパターン (Common AI Regression Patterns)

### パターン 1: サンドボックス/本番パスの不一致 (Pattern 1: Sandbox/Production Path Mismatch)
...
### パターン 2: SELECT 句の省略 (Pattern 2: SELECT Clause Omission)
...
### パターン 3: エラー状態の漏洩 (Pattern 3: Error State Leakage)
...
### パターン 4: 適切なロールバックなしのオプティミスティックアップデート (Pattern 4: Optimistic Update Without Proper Rollback)
...
## 戦略: バグが見つかった場所でテストする (Strategy: Test Where Bugs Were Found)
