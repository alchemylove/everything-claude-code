...
description: "[DEPRECATED - use continuous-learning-v2] レガシー v1 の Stop フック型スキル抽出。v2 はインスティンクトベース・プロジェクトスコープ・フック信頼性を備えた厳密なスーパーセット。v1 は呼び出さず、continuous learning、session learning、pattern extraction は continuous-learning-v2 へルーティングする。"
---

# 継続学習スキル (Continuous Learning Skill - DEPRECATED)

Claude Codeセッションを終了時に自動的に評価し、学習済みスキルとして保存できる再利用可能なパターンを抽出します。

## 動作原理 (Original v1 Documentation)
