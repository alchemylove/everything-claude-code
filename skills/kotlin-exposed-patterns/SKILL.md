...
# Kotlin Exposed パターン (Kotlin Exposed Patterns)
...
### DAO エンティティの使用 (DAO Entity Usage)
...
### HikariCP 設定 (HikariCP Configuration)
...
## データベースセットアップ (Database Setup)

### HikariCP 接続プーリング (HikariCP Connection Pooling)
...
### Flyway マイグレーション (Flyway Migrations)
...
### DSL スタイルのテーブル (DSL Style Tables)
...
### サスペンドトランザクションのサポート (Suspend Transaction Support)
...
### Exposed 実装 (Exposed Implementation)
...
## JSON カラム (JSON Columns)

### kotlinx.serialization を使用した JSONB (JSONB with kotlinx.serialization)
...
## Exposed でのテスト (Testing with Exposed)

### テスト用インメモリデータベース (In-Memory Database for Tests)
...
## Gradle 依存関係 (Gradle Dependencies)
...
## クイックリファレンス: Exposed パターン (Quick Reference: Exposed Patterns)
