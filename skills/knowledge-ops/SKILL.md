...
## ナレッジアーキテクチャ (Knowledge Architecture)

### レイヤー 1: アクティブな実行の真実 (Layer 1: Active execution truth)
...
### レイヤー 2: Claude Code メモリ（クイックアクセス） (Layer 2: Claude Code Memory)
...
### レイヤー 3: MCP メモリサーバー（構造化ナレッジグラフ） (Layer 3: MCP Memory Server)
...
### レイヤー 4: ナレッジベースリポジトリ / 耐久性ドキュメントストア (Layer 4: Knowledge base repo / durable document store)
- **用途:** キュレートされた耐久性ノート、セッションエクスポート、合成されたリサーチ、オペレーターメモリ、長文ドキュメント
- **ルール:** コンテンツがリポジトリ所有のコードでない場合の、クロスマシンコンテキストの優先耐久性ストア

### レイヤー 5: 外部データストア（Supabase、PostgreSQL など） (Layer 5: External Data Store)
- **用途:** 構造化データ、大規模ドキュメントストレージ、全文検索
- **最適な場面:** メモリファイルには大きすぎるドキュメント、SQL クエリが必要なデータ

### レイヤー 6: ローカルコンテキスト/アーカイブフォルダ (Layer 6: Local context/archive folder)
...
### ワークスペース状態の同期 (Workspace State Sync)
...
### GitHub / Linear の同期 (GitHub / Linear Sync)
情報がアクティブな実行に影響する場合:
...
### クロスソースナレッジの同期 (Cross-Source Knowledge Sync)
複数のソースから一箇所に知識を集める:
