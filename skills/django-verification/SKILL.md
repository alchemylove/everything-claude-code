...
# Django 検証ループ (Django Verification Loop)

PR前、大きな変更後、デプロイ前に実行して、Djangoアプリケーションの品質とセキュリティを確保します。

## フェーズ1: 環境チェック (When to Activate)
...
## フェーズ2: コード品質とフォーマット (Phase 1: Environment Check)
...
## フェーズ3: マイグレーション (Phase 2: Code Quality & Formatting)
...
## フェーズ4: テスト + カバレッジ (Phase 3: Migrations)
...
## フェーズ5: セキュリティスキャン (Phase 4: Tests + Coverage)
...
## フェーズ6: Django管理コマンド (Phase 5: Security Scan)
...
## フェーズ7: パフォーマンスチェック (Phase 6: Django Management Commands)
...
## フェーズ8: 静的アセット (Phase 7: Performance Checks)
...
## フェーズ9: 構成レビュー (Phase 8: Static Assets)
...
## フェーズ10: ログ設定 (Phase 9: Configuration Review)
...
## フェーズ11: APIドキュメント（DRFの場合） (Phase 10: Logging Configuration)
...
## フェーズ12: 差分レビュー (Phase 11: API Documentation)
...
## 継続的インテグレーション (Pre-Deployment Checklist)

### GitHub Actionsの例 (Continuous Integration)
...
## クイックリファレンス (GitHub Actions Example)
